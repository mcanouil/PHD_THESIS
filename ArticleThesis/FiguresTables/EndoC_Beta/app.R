...
ui <- dashboardPage(
    skin = "blue",
    dashboardHeader(
        title = HTML(paste0("EndoC &beta; (v", appVersion, ")")),
        dropdownMenuOutput("messageMenu")
    ),
    dashboardSidebar(
        sidebarUserPanel(
            name = a(tags$i(style = "color:#777777", icon("envelope", lib = "glyphicon")), "Amélie Bonnefond", href = 'mailto:amelie.bonnefond@cnrs.fr'),
            subtitle = tags$span(style = "color:#777777", "(Principal Investigator)")
        ),
        sidebarUserPanel(
            name = a(tags$i(style = "color:#777777", icon("envelope", lib = "glyphicon")), "Mickaël Canouil", href = 'mailto:mickael.canouil@cnrs.fr'),
            subtitle = tags$span(style = "color:#777777", "(Biostatistician)")
        ),
        hr(),
        HTML(
    ...
    dashboardBody(
        ...
        tabItems(
            ...
            tabItem(tabName = "importexcelfiles",
                fluidRow(
                ...
                fluidRow(
                    ...
                    box(
                        ...
                        column(3,
                            tags$div(style = "text-align:center margin-top:", actionButton(inputId = "delete", label = "Delete File", width = "100%", icon = icon("trash", lib = "glyphicon")))
                        ),
                        width = 6,
                        collapsible = FALSE,
            ...
            tabItem(tabName = "CorrectionSummary",
                fluidRow(
                    box(
                        HTML(
                            '<div style="text-align:center">
...
server <- function (input, output) {
                ...
                if (length(list.files("www/EXCEL"))!=0) {
                    ...
                    for (iFile in seq_len(nrow(inFiles))) {
                        ...
                        for (isheetname in listsheets) {
                            ...
                            pos <- unlist(apply(cbind(postablename+1, c(postablename[-1]-1, max(postablename)+7)), 1, list), recursive = FALSE)
                            ...
                            save(list = c("operator", info, "Blank"), file = paste0("www/CT/", gsub(".xlsx", "", inFile$Name), "_Sheet_", isheetname, ".Rdata"))
                        }

                        checkrdata <- any(!apply(sapply(list.files(path = "www/CT", pattern = ".*.Rdata", full.names = TRUE), function (xfile) {
                            tmp <- new.env()
                        ...
                        if (file.exists("www/importedfilesinfo.Rdata")) {
                            ...
                            if (inFile[, "Name"]%in%importedfilesinfo[, "Name"]) {
                                newFile <- cbind.data.frame(Name = inFile[, "Name"], Sheets = length(listsheets), Operator = operator, Check = as.integer(0))
                                importedfilesinfo[importedfilesinfo[, "Name"]%in%inFile[, "Name"], ] <- newFile
                            } else {
                                importedfilesinfo <- unique(rbind(
                                    importedfilesinfo,
                                    cbind.data.frame(Name = inFile[, "Name"], Sheets = length(listsheets), Operator = operator, Check = as.integer(0))
                                ))
                            }
                            save(importedfilesinfo, file = "www/importedfilesinfo.Rdata")
                        } else {
                            importedfilesinfo <- cbind.data.frame(Name = inFile[, "Name"], Sheets = length(listsheets), Operator = operator, Check = as.integer(0))
                            save(importedfilesinfo, file = "www/importedfilesinfo.Rdata")
                        }
                    }
                    return(importedfilesinfo)
    ...
    computeQC <- reactive({
        ...
        withProgress(message = "Loading...", value = NULL, {
            ...
            out[, "Warning"] <- as.character(out[, "ExpType"])%in%names(which(by(abs(out[, "intercept"]) > abs(mean(out[, "intercept"]))+IQR(out[, "intercept"]), out[, "ExpType"], all))) |
                as.character(out[, "ExpType"])%in%names(which(by(abs(out[, "slope"]) > abs(mean(out[, "slope"]))+3*IQR(out[, "slope"]), out[, "ExpType"], all)))
            out[, "TypeWarning"] <- factor(paste(out[, "Type"], out[, "Warning"], sep = ":"), levels = c("COR:FALSE", "COR:TRUE", "RAW:FALSE", "RAW:TRUE"))
            ...
            technicalNTP <- unique(do.call("rbind", strsplit(gsub(".*-(.*)-(.*)-.*", "\\1-\\2", rownames(checkNTP[checkNTP[, "FALSE"]>1, ])), "-")))
    ...
    QC_plot1 <- reactive({
        ...
        return(
            ggplot(data = blankdta, aes(x = ID, y = ODdiff, colour = factor(Group))) +
    ...
    output$QC_plot1 <- renderPlot({ QC_plot1() })
    output$QC_plot1download <- downloadHandler(
        filename = function () {
            paste0("RawOD_RE_Insulin.",  input$plotformat)
        },
        content = function (file) {
            ggsave(file = file, plot = QC_plot1() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    QC_plot2 <- reactive({
        validate(need(file.exists("www/importedfilesinfo.Rdata"), "No data available!"))
        ...
        thresholdBlank <- computeQC()[["thresholdBlank"]]
        return(
            ggplot(data = blankdta, aes(x = ID, y = ODdiffCor, colour = factor(Group))) +
                theme_minimal(base_size = input$fontsize) +
    ...
    output$QC_plot2 <- renderPlot({ QC_plot2() })
    output$QC_plot2download <- downloadHandler(
        filename = function () {
            paste0("CorOD_RE_Insulin.",  input$plotformat)
        },
        content = function (file) {
            ggsave(file = file, plot = QC_plot2() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    QC_plot3 <- reactive({
        validate(need(file.exists("www/importedfilesinfo.Rdata"), "No data available!"))
        ...
        thresholdBlank <- computeQC()[["thresholdBlank"]]
        out <- computeQC()[["out"]]
        dtaplot <- merge(subset(blankdta, OD!=0), out[, c("Exp", "ExpType", "Warning", "Type", "intercept", "slope")], by = c("Exp", "ExpType"))
        dtaplot[, "Type"] <- factor(gsub("COR", "CORRECTED", dtaplot[, "Type"]), levels = c("RAW", "CORRECTED"))
        return(
            ggplot(data = subset(dtaplot, abs(ODdiff)<=thresholdBlank), aes(y = log10(OD), x = log10(mUL/23), colour = Warning)) + theme_minimal(base_size = input$fontsize) +
                geom_point(shape = 4) +
                geom_abline(aes(intercept = intercept, slope = slope, colour = Warning)) +
    ...
    output$QC_plot3 <- renderPlot({ QC_plot3() })
    output$QC_plot3download <- downloadHandler(
        filename = function () {
            paste0("LM_OD_Insulin.",  input$plotformat)
        },
        content = function (file) {
            ggsave(file = file, plot = QC_plot3() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    QC_plot4 <- reactive({
        validate(need(file.exists("www/importedfilesinfo.Rdata"), "No data available!"))
        ...
        return(
            ggplot(data = blankdta) + theme_minimal(base_size = input$fontsize) +
                geom_density(aes(x = ODdiff, y = ..scaled..), colour = myPalette()[1], fill = myPalette()[1], alpha = 0.5) +
                geom_vline(xintercept = thresholdBlank, linetype = 2, colour = myPalette()[3]) +
                geom_vline(xintercept = -thresholdBlank, linetype = 2, colour = myPalette()[3]) +
                geom_text(label = percent(thresholdBlank), x = thresholdBlank, y = 1, colour = myPalette()[3], hjust = 1.05, vjust = 1, size = 6) +
                geom_text(label = paste0("-", percent(thresholdBlank)), x = -thresholdBlank, y = 1, colour = myPalette()[3], hjust = -0.05, vjust = 1, size = 6) +
    ...
    output$QC_plot4 <- renderPlot({ QC_plot4() })
    output$QC_plot4download <- downloadHandler(
        filename = function () {
            paste0("RE_DistBlank.",  input$plotformat)
        },
        content = function (file) {
            ggsave(file = file, plot = QC_plot4() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    QC_plot5 <- reactive({
        validate(need(file.exists("www/importedfilesinfo.Rdata"), "No data available!"))
        ...
        scalex <- range(c(blankdta[, "ODdiff"], dataset[, "ODdiff"]), na.rm = TRUE)
        return(
            ggplot(data = dataset) + theme_minimal(base_size = input$fontsize) +
                geom_density(aes(x = ODdiff, y = ..scaled..), colour = myPalette()[1], fill = myPalette()[1], alpha = 0.5) +
                geom_vline(xintercept = thresholdCell, linetype = 2, colour = myPalette()[3]) +
                geom_vline(xintercept = -thresholdCell, linetype = 2, colour = myPalette()[3]) +
                geom_text(label = percent(thresholdCell), x = thresholdCell, y = 1, colour = myPalette()[3], hjust = 1.05, vjust = 1, size = 6) +
                geom_text(label = paste0("-", percent(thresholdCell)), x = -thresholdCell, y = 1, colour = myPalette()[3], hjust = -0.05, vjust = 1, size = 6) +
                scale_x_continuous(labels = percent, limits = scalex) +
                labs(x = bquote("Relative error"~((OD[2]-OD[1])/OD[1])), y = "Density") +
                theme(
                    axis.text = element_text(colour = myPalette()[1], hjust = 0.65, face = "bold")
    ...
    output$QC_plot5 <- renderPlot({ QC_plot5() })
    output$QC_plot5download <- downloadHandler(
        filename = function () {
            paste0("RE_DistExp.",  input$plotformat)
        },
        content = function (file) {
            ggsave(file = file, plot = QC_plot5() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
            ...
            return(
                lapply(unique(dataset[, "Group"]), function (iNTP) {
                    ...
                    XX[, "ug insulin/million cells"] <- sapply(seq_len(nrow(XX)), function (i) {
                        switch(XX[i, "What"],
                            "LYSAT" = {XX[i, "Total content (SN1)"]/50},
                            "SN1" = {NA},
                            "SN2" = {NA}
                        )
                    })
                    XX[, "Insulin"] <- sapply(seq_len(nrow(XX)), function (i) {
                        switch(XX[i, "What"],
                            "LYSAT" = {NA},
                            "SN1" = {(XX[i, "Total ng (in 50 ul)"]*100)/XX[i, "Total content (SN1)"]},
                            "SN2" = {(XX[i, "Total ng (in 50 ul)"]*100)/XX[i, "Total content (SN2)"]}
                        )
                    })
                    XX <- subset(XX, What!="LYSAT", c("Samples", "OD1", "OD2", "What", "Exp", "ExpType", "Group", "Op", "ODdiffCor", "Insulin"))
                    XX[, "Samples"] <- factor(XX[, "Samples"], levels = c("0,5_mM_Glc", "16,7_mM_Glc", "0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX"))
                    return(XX)
                })
            )
        })
    ...
    QCsummarytableReac <- reactive({
        ...
        NTPfail <- do.call("rbind", lapply(NTPdata, function (dta) {
            ...
            return(cbind.data.frame(
                ...
                Fail = !c(all(tab[c("0,5_mM_Glc", "16,7_mM_Glc")]>=2), all(tab[c("0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX")]>=2))
            ))
        }))
        wrongNTPfinal <- merge(NTPfail[which(NTPfail[, "Fail"]), c("Exp", "ExpType")], wrongNTP, by = c("Exp", "ExpType"), all = TRUE)
        lfiles <- as.data.frame(do.call("rbind", strsplit(gsub("(.*)_Sheet_(.*).Rdata", "\\1-\\2", list.files(path = "www/CT", pattern = ".*.Rdata")), "-")))
        ...
        lfiles[, "Status"] <- ifelse(lfiles[, "Group"]%in%paste(wrongNTPfinal[, "Exp"], wrongNTPfinal[, "ExpType"], sep = "-"), "Warning!", "Clear!")
        ...
        withProgress(message = "Loading...", value = NULL, {
            ...
            NTPs <- lapply(NTPdata(), function (iNTPdata) {
                ...
                if (all(tab[c("0,5_mM_Glc", "16,7_mM_Glc")]>=2)) {
                    anovaODA <- tidy(anova(lm(InsulinODvalue~Samples, data = XX, subset = Samples%in%c("0,5_mM_Glc", "16,7_mM_Glc"))))[1, "p.value"]
                } else {
                    anovaODA <- NA
                }
                if (all(tab[c("0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX")]>=2)) {
                    anovaODB <- tidy(anova(lm(InsulinODvalue~Samples, data = XX, subset = Samples%in%c("0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX"))))[1, "p.value"]
                } else {
                    anovaODB <- NA
                }
                anovaAnnot <- cbind.data.frame(
                    ...
                    y = ifelse(is.na(c(anovaODA, anovaODB)), NA, c(
                        max(dtaXX[dtaXX[, "Samples"]%in%c("0,5_mM_Glc", "16,7_mM_Glc"), "Mean"])+max(dtaXX[dtaXX[, "Samples"]%in%c("0,5_mM_Glc", "16,7_mM_Glc"), "Sd"]),
                        max(dtaXX[dtaXX[, "Samples"]%in%c("0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX"), "Mean"])+max(dtaXX[dtaXX[, "Samples"]%in%c("0,5_mM_Glc_+_IBMX", "16,7_mM_Glc_+_IBMX"), "Sd"])
                    )),
                    xmin = c(1, 3),
                    x = c(1.5, 3.5),
                    xmax = c(2, 4),
            ...
            names(NTPs) <- sapply(NTPdata(), function (iNTPdata) { paste(unique(iNTPdata[, c("Exp", "ExpType")]), collapse = "-") })
            return(NTPs)
        })
    })
    ...
    output$CheckBoxGroupUi <- renderUI({
        ...
        return(
            radioButtons(
                inputId = "NTPgroup",
                label = "Group By:",
                choices = list("Target" = "Sheet", "File" = "File"), # list("All" = "All", "File" = "File", "Target" = "Sheet", "Operator" = "Operator"),
                selected = "Sheet",
                inline = TRUE
            )
        )
    ...
    FoldChangeNTP <- reactive({
            ...
            out <- cbind.data.frame(
                Samples = c("Glc", "Glc+IBMX"),
                # Mean = rbind(
                    # rowMeans(iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc"), c("OD1", "OD2")])/rowMeans(iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc"), c("OD1", "OD2")]),
                    # rowMeans(iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc_+_IBMX"), c("OD1", "OD2")])/rowMeans(iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc_+_IBMX"), c("OD1", "OD2")])
                # ),
                Mean = rbind(
                    iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc"), "Insulin"]/iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc"), "Insulin"],
                    iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc_+_IBMX"), "Insulin"]/iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc_+_IBMX"), "Insulin"]
                ),
                Exp = iindex,
        ...
        fcplot <- ggplot(data = subset(foldChangedta, Samples=="Glc")) + theme_minimal(base_size = input$fontsize) +
            geom_density(aes(x = Value, y = ..scaled..), colour = myPalette()[1], fill = myPalette()[1], alpha = 0.5) +
            geom_vline(xintercept = FCthreshold, linetype = 2, colour = myPalette()[3]) +
            geom_text(label = FCthreshold, x = FCthreshold, y = 1, colour = myPalette()[3], hjust = 1.05, vjust = 1, size = 6) +
            scale_x_continuous(limits = range(pretty_breaks()(range(foldChangedta[, "Value"], na.rm = TRUE)), na.rm = TRUE)) +
    ...
    output$FoldChangeNTP_plot <- renderPlot({ FoldChangeNTP() })
    output$FoldChangeNTP_plotdownload <- downloadHandler(
        filename = function () {
            return(paste0("FoldChange_siNTP_Distribution.", input$plotformat))
        },
        content = function (file) {
            ggsave(file = file, plot = FoldChangeNTP_plot() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    ...
    NTPAnalysis_plot1 <- reactive({
        ...
        if (length(unique(gsub("(.*)-(.*)", "\\1", YY[, "Group"])))==1 & length(unique(gsub("(.*)-(.*)", "\\1", out[, "Group"])))==1) {
        ...
        pout2 <- ggplot(data = subset(YY, OD!=0), aes(y = log10(OD), x = log10(mUL/23), colour = Group, shape = Group)) + theme_minimal(base_size = input$fontsize) +
            geom_point(size = 2) +
            geom_abline(data = subset(out, Type=="COR"), aes(intercept = intercept, slope = slope, colour = Group)) +
            labs(y = bquote(log[10](OD)), x = bquote('Insulin'~(log[10](mu*g/L)))) +
            scale_shape_manual(
    ...
    output$NTPAnalysis_plot1 <- renderPlot({ NTPAnalysis_plot1() })
    output$NTPAnalysis_plot1download <- downloadHandler(
        ...
        content = function (file) {
            ggsave(file = file, plot = NTPAnalysis_plot1() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    ...
    NTPAnalysis_plot2 <- reactive({
        ...
        pout <- ggplot() + theme_minimal(base_size = input$fontsize) +
            geom_errorbar(data = dtaXX, aes(x = Samples, y = Mean, ymin = Mean*0.95, ymax = Mean+Sd, colour = Exp, fill = Exp), width = 0.2, position = position_dodge(widthbar)) +
            geom_bar(data = dtaXX, aes(x = Samples, y = Mean, colour = Exp, fill = Exp), stat = "identity", position = "dodge", width = widthbar) +
            scale_colour_manual(
        ...
        if (legendOn) {
            if (!all(is.na(anovaAnnot[, "y"])) & length(palcolors)<=6) {
                ...
                if (input$reportstars) {
                    anovaAnnot[, "labels"] <- factor(sapply(anovaAnnot[, "p"], signstars), levels = c("ns", "ns", "*", "**", "***"))
                    pout <- pout +
                        geom_errorbarh(data = anovaAnnot, aes(x = x, y = y, xmin = xmin, xmax = xmax, colour = Exp), height = 0)
                    if (any(anovaAnnot[, "labels"]%in%"ns")) {
                        pout <- pout +
                            geom_text(data = subset(anovaAnnot, labels%in%"ns"), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = Exp), vjust = -0.25, size = 5)
                    } else {}
                    if (any(!anovaAnnot[, "labels"]%in%c("ns", "ns"))) {
                        pout <- pout +
                            geom_text(data = subset(anovaAnnot, !labels%in%c("ns", "ns")), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = Exp), vjust = 0.40, size = 8)
                    } else {}
                    if (any(anovaAnnot[, "labels"]%in%".")) {
                        pout <- pout +
                            geom_text(data = subset(anovaAnnot, labels%in%"."), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = Exp), vjust = -0.25, size = 10)
                    } else {}
                } else {
                    pout <- pout +
                        geom_errorbarh(data = anovaAnnot, aes(x = x, y = y, xmin = xmin, xmax = xmax, colour = Exp), height = 0) +
                        geom_text(data = anovaAnnot, aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = Exp), vjust = -0.25, size = 5)
                }
    ...
    output$NTPAnalysis_plot2 <- renderPlot({ NTPAnalysis_plot2() })
    output$NTPAnalysis_plot2download <- downloadHandler(
        ...
        content = function (file) {
            ggsave(file = file, plot = NTPAnalysis_plot2() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    ...
    NTPAnalysis_plot2bis <- reactive({
        ...
        foldChangedta <- do.call("rbind", lapply(index, function (iindex) {
            ...
            out <- cbind.data.frame(
                Samples = c("Glc", "Glc+IBMX"),
                Mean = rbind(
                    iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc"), c("Insulin")]/iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc"), c("Insulin")],
                    iNTPdata[iNTPdata[, "Samples"]%in%c("16,7_mM_Glc_+_IBMX"), c("Insulin")]/iNTPdata[iNTPdata[, "Samples"]%in%c("0,5_mM_Glc_+_IBMX"), c("Insulin")]
                ),
                Exp = iindex,
                Op = unique(iNTPdata[, "Op"]),
                ExpType = unique(iNTPdata[, "ExpType"])
        ...
        FullDta <- do.call("rbind", by(index, gsub("-.*", "", index), function (iindex) {
            ...
            ZZZ <- merge(XXX, YYY, by.x = c("Group", "ExpType", "Replicates", "PrettySamples", "Op", "Exp"), by.y = c("Exp", "ExpType", "Replicates", "Samples", "Op", "ExpShort"))
            if (input$FoldChangeUiActivate) {
                checkFCsiNTP <- ZZZ[grep("NTP", ZZZ[, "ExpType"]), ]
                checkFCsiNTP[which(checkFCsiNTP[, "FCValue"]<input$FCthreshold), c("Insulin", "FCValue")] <- NA
                lowFCvalue <- names(which(table(paste(checkFCsiNTP[, "Exp"], checkFCsiNTP[, "Samples"], sep = ":"), factor(!is.na(checkFCsiNTP[, "FCValue"]), levels = c(FALSE, TRUE)))[, "FALSE"]>=2))
        ...
        if (!input$removeoutlier) {
            FullDta <- do.call("rbind", by(FullDta, FullDta[, c("Samples", "ExpType")], function (idta) {
                subset(idta, (Insulin<=median(Insulin, na.rm = TRUE)+1.5*IQR(Insulin, na.rm = TRUE)) & (Insulin>=median(Insulin, na.rm = TRUE)-0.5*IQR(Insulin, na.rm = TRUE)))
            }))
        } else {}
        pout <- ggplot(data = FullDta, aes(x = Samples, y = Insulin, colour = ExpType, fill = ExpType)) + theme_minimal(base_size = input$fontsize) +
            geom_boxplot(fill = "white", outlier.shape = ifelse(input$removeallpoints| !input$removeoutlier, NA, 16))
        if (input$removeallpoints) {
            pout <- pout + geom_point(position = position_jitterdodge(jitter.width = widthbar/2), shape = 4)
            if (input$removelowfC) {
                pout <- pout + geom_point(data = subset(FullDta, LowFC), aes(y = InsulinAll), colour = myPalette()[2], position = position_jitterdodge(jitter.width = widthbar/2), shape = 3)
            } else {}
        } else {}
    ...
    output$NTPAnalysis_plot2bis <- renderPlot({ NTPAnalysis_plot2bis() })
    output$NTPAnalysis_plot2bisdownload <- downloadHandler(
        ...
        content = function (file) {
            ggsave(file = file, plot = NTPAnalysis_plot2bis() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
    ...
    NTPAnalysis_plot3 <- reactive({
        ...
        if (enoughExp) {
            ...
            if (enoughMeasures[1]) {
                ...
                lmRes_NTP_GLC <- as.data.frame(matrix(NA, nrow = 0, ncol = ncol(lmRes_GLC_IBMX), dimnames = list(NULL, colnames(lmRes_GLC_IBMX))))
            }
            if (enoughMeasures[2]) {
                ...
                lmRes_NTP_GLCIBMX <- as.data.frame(matrix(NA, nrow = 0, ncol = ncol(lmRes_GLC_IBMX), dimnames = list(NULL, colnames(lmRes_GLC_IBMX))))
            }
        } else {
            if (length(unique(FullDtaUnique[!is.na(FullDtaUnique[, "Value"]), "Op"]))!=1) {
                if (length(unique(FullDtaUnique[!is.na(FullDtaUnique[, "Value"]), "ExpShort"]))!=1) {
        ...
        if (enoughExp) {
            ...
            lmAnnot <- do.call("rbind", by(lmAnnot, lmAnnot[, "Samples"], function (dta) {
                if (unique(dta[, "Samples"])=="Global") {
                    ...
                    dta[, "y"] <- ifelse(is.na(dta[, "p.value"]), NA, max(rowSums(foldChangedtaplot[foldChangedtaplot[, "Samples"]%in%unique(dta[, "Samples"]), c("Mean", "Sd")])))
                }
                return(dta)
            }))
            ...
            lmAnnot <- do.call("rbind", by(lmAnnot, lmAnnot[, "Samples"], function (dta) {
                if (unique(dta[, "Samples"])=="Glc") {
                    dta[, "xmax"] <- seq(1-widthbar/2, 1+widthbar/2, widthbar/nsize)[as.integer(as.factor(dta[, "ExpType"]))+1] + widthbar/(nsize*2)
                } else {
                    dta[, "xmax"] <- seq(2-widthbar/2, 2+widthbar/2, widthbar/nsize)[as.integer(as.factor(dta[, "ExpType"]))+1] + widthbar/(nsize*2)
                }
                return(dta)
            }))
            ...
            lmAnnot[, "y"] <- c(lmAnnot[, "y"] + max(scaleFCy)*rep(seq_along(unique(lmAnnot[, "ExpType"]))*0.10, 2))[seq_len(nrow(lmAnnot))]
        ...
        pout <- pout + scale_colour_manual(name = paste0(ifelse(tab, "Warning!", "Clear!")), values = unlist(ifelse(length(index)==1, "gray30", list(palcolors)))) +
            scale_fill_manual(name = paste0(ifelse(tab, "Warning!", "Clear!")), values = unlist(ifelse(length(index)==1, "gray30", list(palcolors))))
            ...
            if(!all(is.na(lmAnnot[, "y"]))) {
                if (input$reportstars) {
                    ...
                    if (any(lmAnnot[, "labels"]%in%"ns")) {
                        pout <- pout +
                            geom_text(data = subset(lmAnnot, labels%in%"ns"), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = ExpType), vjust = -0.25, size = 5)
                    } else {}
                    if (any(!lmAnnot[, "labels"]%in%c("ns", "ns"))) {
                        pout <- pout +
                            geom_text(data = subset(lmAnnot, !labels%in%c("ns", "ns")), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = ExpType), vjust = 0.40, size = 8)
                    } else {}
                    if (any(lmAnnot[, "labels"]%in%".")) {
                        pout <- pout +
                            geom_text(data = subset(lmAnnot, labels%in%"."), aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = ExpType), vjust = -0.25, size = 10)
                    } else {}
                } else {
                    pout <- pout +
                        geom_errorbarh(data = lmAnnot, aes(x = x, y = y, xmin = xmin, xmax = xmax, colour = ExpType), height = 0) +
                        geom_text(data = lmAnnot, aes(x = x, y = y, xmin = xmin, xmax = xmax, label = labels, colour = ExpType), vjust = -0.25, size = 5)
                }
    ...
    output$NTPAnalysis_plot3 <- renderPlot({ NTPAnalysis_plot3() })
    output$NTPAnalysis_plot3download <- downloadHandler(
        ...
        content = function (file) {
            ggsave(file = file, plot = NTPAnalysis_plot3() + theme(legend.title = element_blank()), width = input$plotwidth, height = input$plotheight, units = input$plotunits, dpi = as.numeric(input$plotdpi))
        }
    )
