ggdraw <- function (plot = NULL, xlim = c(0, 1), ylim = c(0, 1), theme_dark = FALSE) {
    ...
    p <- ggplot(d, aes_string(x = "x", y = "y")) + 
        ...
        labs(x = NULL, y = NULL)
    if (!is.null(plot)) {
        g <- cowplot:::ggplot_to_gtable(plot)
...
theme_black <- function(base_size = 11, base_family = "", base_line_size = base_size / 22, base_rect_size = base_size / 22) {
    ...
    theme(
        ...
        axis.line.x = NULL,
        axis.line.y = NULL,
        axis.text = element_text(size = rel(0.8), colour = "dodgerblue"),
        ...
        panel.border = element_rect(fill = NA, colour = "grey20", size = 0.5, linetype = "solid"),
        panel.grid.major = element_line(colour = "grey50"),
        panel.grid.minor = element_line(colour = "grey50", size = rel(0.5)),
        panel.spacing = unit(0, "cm"), # unit(half_line, "pt"),
